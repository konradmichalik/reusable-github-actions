...
Generated by `scripts/find-consumers.sh`. Not maintained by hand: re-run the script and replace this file when the list needs refreshing.

Authoritative as of 2026-08-24, produced by walking `.github/workflows/*.yml` of every non-archived repository under `konradmichalik`, `move-elevator` and `xima-media` directly via the GitHub contents API. GitHub's code search index was checked first and found to be non-exhaustive (known consumers were absent from its results), so this list does not rely on it.

## Summary

- **39** consumer repositories, **130** `uses:` references, all currently `@main`
- Owners: konradmichalik, move-elevator, xima-media
- **4** private: konradmichalik/php-sync-tool, konradmichalik/tally, konradmichalik/typo3-translation-validator, move-elevator/typo3-pulse
...
`jackd248` is this project's own former GitHub username before a rename to `konradmichalik`, not a third-party fork. The reference still resolves today via GitHub's username-rename redirect, which is not something to rely on: a future registration of that username by someone else would break it silently. 4 reference(s) across 3 repositories need rewriting to `konradmichalik/reusable-github-actions`.
...
See #39. `tests.yml` is superseded by `tests-typo3.yml` and scheduled for removal in `0.3.0`. Migration is a one-line change of the `uses:` path.
...
| konradmichalik/md-annotator | public | `.github/workflows/release.yml` | `release.yml` | `main` |
...
| konradmichalik/php-sync-tool | private | `.github/workflows/cgl.yml` | `cgl.yml` | `main` |
